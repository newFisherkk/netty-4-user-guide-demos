	...
	public void run() {
		...
		try {
			...
			if (socketChannel != null) {
				...
				SubReactor subReactor = reactors[next];
				subReactor.registering(true);
				Selector selector = selectors[next];
				selector.wakeup(); // 使一个阻塞住的selector操作立即返回
				SelectionKey selectionKey = 
						socketChannel.register(selector, SelectionKey.OP_READ); // 注册一个读事件
				selector.wakeup(); // 使一个阻塞住的selector操作立即返回

				// 本次事件注册完成后，需要再次触发select的执行，
				// 因此这里Restart要在设置回false（具体参考SubReactor里的run方法）
				subReactor.registering(false);

				// 绑定Handler
				selectionKey.attach(new AsyncHandler(socketChannel, selector, next));
